...
DEFAULT_LOG_DIR = Path(
    r"C:\Users\grulovicma\Matija Grulovic\GitHub\SimRLFab_mata\log\80 states FIFO - util"
)
NORMALIZATION = True
FIRST_N = 1000
...
def clean_pairs(x_series: pd.Series, y_series: pd.Series, max_points: int) -> Tuple[List[float], List[float]]:
    pairs = []
    for x_val, y_val in zip(x_series.tolist(), y_series.tolist()):
        try:
            x_num = float(x_val)
            y_num = float(y_val)
...
def build_dashboard(
    ...
    reward_window = max(50, min(2000, max(50, len(agent_df) // 500)))
    reward_trace = agent_df[["sim_step", "reward"]].copy()
    reward_trace = reward_trace.reset_index(drop=True)  # ! added
    reward_trace["global_step"] = range(len(reward_trace))  # ! added
    reward_trace["rolling_reward"] = reward_trace["reward"].rolling(
        window=reward_window,
        min_periods=max(5, reward_window // 10),
    ).mean()
    reward_trace["reward_visualized"] = maybe_normalize_series(reward_trace["reward"])
    reward_trace["rolling_reward_visualized"] = maybe_normalize_series(reward_trace["rolling_reward"])
    ...
    panels = [
            ...
            [
                {
                    ...
                    "x": agent_episode_normalized["episode"][:FIRST_N],
                    "y": agent_episode_normalized["agent_total_reward"][:FIRST_N],
                    "color": COLORS["amber"],
                },
            ],
        ...
        svg_line_chart(
            ...
            [{"label": "finished_orders", "x": episode_df["episode_counter"][:FIRST_N],
            "y": pd.to_numeric(episode_df["finished_orders"][:FIRST_N], errors="coerce"),
            "color": COLORS["green"]}],
            max_points=max_points,
        ),
        svg_line_chart(
            "Order Waiting Time per Episode",
            [{"label": "order_waiting_time", "x": episode_df["episode_counter"][:FIRST_N],
            "y": pd.to_numeric(episode_df["order_waiting_time"][:FIRST_N], errors="coerce"),
            "color": COLORS["red"]}],
            max_points=max_points,
        ),
        svg_line_chart(
        ...
        svg_line_chart(
            "Agent Reward by Decision Step",    # ← no normalization_suffix
            [
                {
                    "label": "reward",
                    "x": reward_trace["global_step"][:FIRST_N],
                    "y": pd.to_numeric(reward_trace["reward"][:FIRST_N], errors="coerce"),   # ← raw, not reward_visualized
                    "color": COLORS["amber"],
                },
                # rolling mean series removed
            ],
            max_points=max_points,
        ),
